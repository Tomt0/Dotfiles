...
-- Avahi service browsers are network tools, not user-facing internet apps
local BLACKLIST = { ["Avahi SSH Server Browser"]=true, ["Avahi VNC Server Browser"]=true, ["Avahi Zeroconf Browser"]=true }
      ...
      for path in h:lines() do
        ...
        then
          ...
          if name ~= "" and not seen[name] and not BLACKLIST[name]
            and has_cat(cats, "Network", "WebBrowser", "Email", "InstantMessaging", "Chat")
          then
            seen[name] = true
            table.insert(apps, {
              Text    = name,
              Icon    = e.Icon or "applications-internet",
