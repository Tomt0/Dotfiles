...
-- Qt internal tooling that isn't a general-purpose dev app
local BLACKLIST = {
  ["Qt Assistant"]=true, ["Qt D-Bus Viewer"]=true, ["Qt Linguist"]=true,
  ["Qt Widgets Designer"]=true, ["Electron 37"]=true, ["Icon Browser"]=true,
}
      ...
      for path in h:lines() do
        ...
        then
          ...
          if name ~= "" and not seen[name] and not BLACKLIST[name]
            and has_cat(cats, "Development", "IDE", "WebDevelopment", "Debugger", "TextEditor")
          then
            seen[name] = true
            table.insert(apps, {
              Text    = name,
              Icon    = e.Icon or "applications-development",
