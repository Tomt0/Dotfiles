...
setup_dotfiles_apply() {
    ...
    local dirs=(
        ...
        cava mpv nwg-look waypaper viegphunt elephant colors
    )
    for d in "${dirs[@]}"; do
    ...
    local copy_dirs=(
        ...
        qt5ct qt6ct Kvantum gtk-3.0 gtk-4.0 wlogout
        cava mpv nwg-look waypaper elephant colors
    )
    ...
    # viegphunt: merge scripts from dotfiles without removing walker_start.sh
    # (walker_start.sh is generated by setup_scripts() and is not in the repo)
    if [[ -d "$dotfiles/.config/viegphunt" ]]; then
        mkdir -p "$HOME/.config/viegphunt"
        for f in "$dotfiles/.config/viegphunt"/*.sh; do
            [[ -f "$f" ]] || continue
            cp "$f" "$HOME/.config/viegphunt/$(basename "$f")"
            chmod +x "$HOME/.config/viegphunt/$(basename "$f")"
            ok "viegphunt/$(basename "$f")"
        done
    fi
